...
@section('content')
    <section class="w-full h-full flex flex-col">
        <div class=" flex p-4 h-auto">
            ...
            <div class="mt-auto">
                <span class="mx-auto my-4 mt-auto text-gray-700 font-bold">Manage<br>Monsakun</span>
            </div>
            <div class="mt-auto ml-auto gap-4 flex flex-row">
        ...
        <div class="w-full h-screen p-8">
            <span class="text-lg text-gray-800 font-bold"><i class="fa-solid fa-users-between-lines"></i> Pilihan Kelas</span>
            <div class="grid grid-cols-5 gap-4 p-4">
                @foreach ($class as $item)
                    <div class="shadow-md rounded-xl h-[250px] flex flex-col relative bg-gray-50">
                        <span class="mx-auto mt-auto text-xl font-bold text-gray-600">Kelas {{$item['no_class']}}</span>
                        <div class="flex flex-col mt-auto h-[30%] px-2 text-sm justify-center">
                            <span class=""><i class="fa-solid fa-list-check"></i> Total Soal : {{$item['sum_question']}}</span>    
                            <span class=""><i class="fa-solid fa-eraser"></i> Pengurangan Point : {{$item['remove_point']}}</span>
                        </div>
                        <div class="flex h-[20%] text-white justify-center rounded-b-lg">
                            <a href="{{route('rankingClass', $item['id_class'])}}" class="w-2/4 bg-gray-200 h-full rounded-bl-lg flex">
                                <i class="fa fa-trophy text-yellow-600 m-auto"></i>
                            </a>
                            <a href="{{route('editClass', $item['id_class'])}}" class="w-2/4 bg-yellow-400 h-full rounded-br-lg flex">
                                <i class="fa-solid fa-edit text-black m-auto"></i>
                            </a>
                        </div>
                    </div>                   
                @endforeach
