    ...
    <section class="bg-blue-400 w-full h-full flex">
        <div class="w-[400px] h-auto bg-white m-auto rounded-xl shadow-xl flex flex-col p-4">
            <div class="flex flex-col ">
                ...
                <span class="mx-auto my-4 font-bold">Guru - Daftar</span>
            </div>
            <form action="{{route('register_process')}}" method="POST" class="w-full grid grid-cols-1 gap-4">
                @csrf
                <div class="flex flex-col justify-center w-full gap-1">
                    <label for="name" class="text-sm">Nama Lengkap</label>
                    <input type="text" name="name" placeholder="Masukan Nama Lengkap" class="p-2 bg-gray-100 text-sm focus:outline-none" required>
                </div>
                <div class="flex flex-col justify-center w-full gap-1">
                    <label for="email" class="text-sm">Email</label>
                    <input type="email" name="email" placeholder="Masukan Email" class="p-2 bg-gray-100 text-sm focus:outline-none" required>
                </div>
                <div class="flex flex-col justify-center w-full gap-1">
                    <label for="password" class="text-sm">Password</label>
                    <input type="password" name="password" placeholder="Masukan Password" class="p-2 bg-gray-100 text-sm focus:outline-none" required>
                </div>
                <div class="flex flex-col justify-center w-full gap-1">
                    <label for="no_telepon" class="text-sm">No Telepon</label>
                    <input type="text" name="no_telepon" placeholder="Masukan No Telepon" class="p-2 bg-gray-100 text-sm focus:outline-none" required>
                </div>
                <div class="flex flex-col justify-center w-full gap-1">
                    <input type="submit" value="Register" class="p-2 bg-gray-100 cursor-pointer hover:bg-blue-400 hover:text-white rounded-md text-sm" value="Register">
                    <a href="{{url('teacher/login')}}" class="p-2 cursor-pointer text-blue-400 rounded-md text-center text-sm hover:underline">Login</a>
                </div>
            </form>
        </div>
    </section>
